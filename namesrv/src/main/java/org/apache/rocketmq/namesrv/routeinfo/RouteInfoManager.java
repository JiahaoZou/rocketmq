...
public class RouteInfoManager {
    ...
    // 每个topic的所有queue信息，包含了每个queue所属的brokerName。一个queue结构体对应了一个topic在一个broker上的信息
    // 一个topic在一个broker上可能存在多个写queue和读queue，queue信息由broker的注册带过来
    private final HashMap<String/* brokerName */, BrokerData> brokerAddrTable;
    // brokerData结构体里维护了brokername，所属cluster，以及一个hashmap id->（ip：port）
    // 一个broker集群->多个broker组（brokerName）-> 一个brokerName-> 多个broker机器（主从复制）
    // rmq的主从是broker级别的
    private final HashMap<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;
    // 一个namesrv可以管理多个cluster，但通常来说会为每个cluster单独起一个独立namesrv
    private final HashMap<String/* brokerAddr */, BrokerLiveInfo> brokerLiveTable;
    // 用于管理broker长链接，是否有心跳，key为实际的broker机器
    private final HashMap<String/* brokerAddr */, List<String>/* Filter Server */> filterServerTable;
    // 在每台broker机器上可以启动一个或多个filterserver，启动后会跟本地的broker进行长链接构建。broker和filter的连接是在一台机器上。
    // 我们可以先自定义个一个class，然后吧class上传到filter server里，在消费数据时，先要broker把数据传输到broket本机的filter server里，
    // 由filter server基于上传的自定义的class来进行细粒度的筛选。
    // 这种筛选不依赖与tag构建的索引，字节rmq平台的搜索功能就是如此实现的。
    ...
    // 定时保活，broker跟namesrv之间超过BROKER_CHANNEL_EXPIRED_TIM都没有通信，关闭连接，以及清理相关数据结构
...
class BrokerLiveInfo {
    private long lastUpdateTimestamp; // broker主动上报心跳时更新时间戳
    private DataVersion dataVersion; // 数据版本号
    private Channel channel;  // netty的channel，长链接
    private String haServerAddr; // HA 高可用的broker机器地址
